        ...
        <div className={cn(
            ...
            <div className="flex items-center gap-4">
                <button 
                    onClick={togglePlay}
                    disabled={loadError}
                    className="h-12 w-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-xl shadow-primary/20 hover:scale-110 active:scale-90 transition-all"
                >
                ...
                <div className="h-14 w-14 rounded-[1.5rem] bg-primary/10 flex items-center justify-center text-primary shrink-0 relative overflow-hidden shadow-inner border border-primary/5">
                    {loadError ? <AlertCircle className="h-7 w-7 text-destructive" /> : <Music className={cn("h-7 w-7 transition-transform", isPlaying && "animate-bounce")} />}
                </div>
            </div>

            <div className="flex items-center gap-4 pt-1">
                <button onClick={onRemove} className="h-9 w-9 rounded-full bg-destructive/10 flex items-center justify-center text-destructive transition-all active:scale-90 shadow-sm border border-destructive/5"><Trash2 className="h-4 w-4" /></button>
                <div className="flex-1 px-1">
                    <Slider value={[currentTime]} max={duration || 100} step={0.1} onValueChange={(v) => { if(audioRef.current) audioRef.current.currentTime = v[0]; }} className="cursor-pointer" disabled={loadError} />
                </div>
            </div>
        </div>
    );
  ...
  const renderFavoriteItem = (item: FavoriteItem) => {
    ...
    return (
        <div key={item.id} className="flex flex-col gap-4 animate-in fade-in duration-500">
            ...
            <Button className="w-full rounded-[1.5rem] h-14 font-black shadow-xl text-sm" onClick={() => handleAction(item, () => item.downloadUrl && window.open(item.downloadUrl, '_blank'))}>
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-secondary">
      ...
      <main className="flex-1 px-6 pt-4 pb-24">
        {favorites.length > 0 ? (
          <div className="space-y-12 mt-6">{favorites.map((item) => renderFavoriteItem(item))}</div>
        ) : (
      ...
      <Dialog open={!!selectedImage} onOpenChange={(open) => !open && setSelectedImage(null)}>
        <DialogContent className="max-w-4xl p-0 bg-transparent border-0 shadow-none overflow-hidden rounded-3xl">
          ...
          {selectedImage && <div className="relative w-full h-[85vh]"><img src={selectedImage} alt="Preview" className="w-full h-full object-contain" /></div>}
        </DialogContent>
      </Dialog>
    </div>
  );
