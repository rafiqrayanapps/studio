...
export default function CategoryError({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-secondary">
        <Header showMenu={false} title={t('errorLoadingCategory')}>
        ...
        <main className="flex-1 flex items-center justify-center px-6 pb-24 -mt-12">
            <div className="bg-card p-8 rounded-2xl shadow-lg max-w-md w-full text-center space-y-4">
                ...
                <h2 className="text-2xl font-bold text-foreground">{t('categoryUnderMaintenance')}</h2>
                <p className="text-muted-foreground">
                {t('categoryUnderMaintenanceMessage')}
                </p>
                <Button
                onClick={() => reset()}
                className="mt-4"
