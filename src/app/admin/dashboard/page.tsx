...
import type { Category, ContentItem, WhitelistEntry, ReferralConfig, ThemeConfig, SubscriptionDialogConfig, Notification, PaymentMethod, PricingPlan } from '@/lib/definitions';
...
export default function AdminDashboard() {
  ...
  const whitelistQuery = useMemoFirebase(() => {
      if (!firestore) return null;
      return query(collection(firestore, 'whitelist'));
  }, [firestore]);
  ...
  const notificationsQuery = useMemoFirebase(() => {
      if (!firestore) return null;
      return query(collection(firestore, 'notifications'));
  }, [firestore]);
  const { data: notifications } = useCollection<Notification>(notificationsQuery);

  const paymentsQuery = useMemoFirebase(() => {
      if (!firestore) return null;
      return query(collection(firestore, 'paymentMethods'));
  }, [firestore]);
  const { data: rawPayments } = useCollection<PaymentMethod>(paymentsQuery);
  const paymentMethods = useMemo(() => {
      if (!rawPayments) return null;
      return [...rawPayments].sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
  }, [rawPayments]);

  const plansQuery = useMemoFirebase(() => {
      if (!firestore) return null;
      return query(collection(firestore, 'pricingPlans'));
  }, [firestore]);
  const { data: rawPlans } = useCollection<PricingPlan>(plansQuery);
  const pricingPlans = useMemo(() => {
      if (!rawPlans) return null;
      return [...rawPlans].sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
  }, [rawPlans]);
  ...
  const onUpdateCategory = async (values: z.infer<typeof categorySchema>) => {
      ...
      try {
          ...
          const finalOrder = isNew ? (categories?.length || 0) : (editingCategory.order ?? 0);
  ...
  const onUpdatePlan = async (values: z.infer<typeof pricingPlanSchema>) => {
      if (!firestore) return;
      try {
          const planId = (editingPlan?.id) ? editingPlan.id : doc(collection(firestore, 'pricingPlans')).id;
          const featuresArray = values.features.split(',').map(f => f.trim()).filter(f => f !== '');
          
          await setDoc(doc(firestore, 'pricingPlans', planId), {
              ...values,
              features: featuresArray,
              order: editingPlan?.order ?? (pricingPlans?.length || 0)
          }, { merge: true });
  ...
  const onUpdatePayment = async (values: z.infer<typeof paymentMethodSchema>) => {
      if (!firestore) return;
      try {
          const methodId = (editingPayment?.id) ? editingPayment.id : doc(collection(firestore, 'paymentMethods')).id;
          await setDoc(doc(firestore, 'paymentMethods', methodId), {
              ...values,
              order: editingPayment?.order ?? (paymentMethods?.length || 0)
          }, { merge: true });
    ...
    <div className="flex min-h-screen bg-secondary/30">
      <aside className="hidden lg:flex w-72 flex-col bg-card border-l sticky top-0 h-screen shadow-xl z-50">
        <div className="p-8 border-b flex flex-col items-center gap-3">
            <div className="bg-primary text-primary-foreground p-4 rounded-[2rem] shadow-2xl shadow-primary/20 rotate-3">
                <LayoutDashboard className="h-10 w-10" />
            </div>
            <div className="text-center">
                <h1 className="font-black text-2xl tracking-tighter">لوحة التحكم</h1>
                <Badge variant="secondary" className="mt-1 rounded-full px-4">
                    {isAdmin ? "مدير النظام" : "محرر محتوى"}
                </Badge>
            </div>
        </div>
            ...
            <nav className="space-y-2">
                {menuItems.map(item => (
                    ...
                    >
                        ...
                        {item.badge ? (
                            <span className="mr-auto bg-destructive text-[10px] px-2 py-0.5 rounded-full text-white animate-pulse">
                                {item.badge}
                            </span>
                        ) : null}
                    </button>
        ...
        <div className="p-6 border-t bg-muted/20">
            <Button 
                variant="outline" 
                className="w-full h-12 rounded-2xl border-2 font-bold hover:bg-primary hover:text-primary-foreground transition-all" 
                onClick={() => router.push('/home')}
            >
                <ArrowLeft className="ml-2 h-4 w-4" /> العودة للتطبيق
            </Button>
        </div>
      </aside>
                        ...
                        <Card key={cat.id} className={`overflow-hidden border-2 transition-all duration-300 ${cat.isUnderMaintenance ? 'border-orange-500/30 bg-orange-50/10' : 'border-primary/5'}`}>
                            <CardHeader className="p-6 bg-muted/20 flex-row justify-between items-start space-y-0">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex flex-col gap-0.5">
                                            <Button 
                                                ...
                                                disabled={idx === 0} 
                                                onClick={() => moveItemInFirestore(categories, idx, 'up', ['categories'])}
                                            >
                                            ...
                                            <Button 
                                                ...
                                                onClick={() => moveItemInFirestore(categories, idx, 'down', ['categories'])}
                                            >
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {cat.fileTypes && (
                                            <Badge className="bg-primary/10 text-primary text-[8px] font-black h-5 border-primary/20">
                                                {cat.fileTypes}
                                            </Badge>
                                        )}
                                    </div>
                                </div>
                                <div className="bg-background p-2 rounded-2xl shadow-sm border flex-shrink-0">
                            ...
                            <CardContent className="p-6 pt-4 border-t bg-card/50">
                                <div className="flex items-center justify-between p-4 bg-background rounded-2xl border shadow-inner">
                                    ...
                                    <Switch checked={cat.isUnderMaintenance} onCheckedChange={() => updateDocumentNonBlocking(doc(firestore!, 'categories', cat.id), { isUnderMaintenance: !cat.isUnderMaintenance })} />
                                </div>
                            </CardContent>
                            <CardFooter className="p-3 border-t flex gap-2 bg-muted/10">
                                <Button variant="secondary" className="text-xs font-bold flex-1 h-10 rounded-xl" onClick={() => { setSelectedCategoryId(cat.id); setActiveTab('items'); }}>
                                    <FileText className="ml-2 h-4 w-4" /> إدارة المحتوى
                                </Button>
                                <Button variant="ghost" size="icon" className="h-10 w-10 text-primary rounded-xl hover:bg-primary/10" onClick={() => { setEditingCategory(cat); catForm.reset(cat); }}>
                                ...
                                {isAdmin && (
                                    <Button 
                                        ...
                                        onClick={() => confirm("حذف القسم وكافة محتوياته؟") && deleteDocumentNonBlocking(doc(firestore!, 'categories', cat.id))}
                                    >
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                    <Card className="xl:col-span-5 shadow-2xl shadow-primary/5 rounded-[2.5rem] border-none">
                        <CardHeader className="p-8 pb-0">
                            <CardTitle className="text-2xl font-black">إضافة محتوى جديد</CardTitle>
                            <CardDescription>املأ البيانات لنشر محتوى جديد في القسم المختار.</CardDescription>
                        </CardHeader>
                            ...
                            <div className="space-y-4">
                                ...
                                {selectedCategoryId && (
                                    <div className="space-y-2 animate-in fade-in zoom-in duration-300">
                                        <label className="text-sm font-black flex items-center gap-2 px-1">
                                            <Palette className="h-4 w-4 text-primary" /> نمط العرض
                                        </label>
                                ...
                                <Form {...itemForm}>
                                    <form onSubmit={itemForm.handleSubmit(onAddItem)} className="space-y-6 pt-6 border-t">
                                        <FormField 
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel className="font-black">عنوان المنشور</FormLabel>
                                                    <FormControl>
                                                        <Input {...field} className="h-12 rounded-xl" placeholder="مثال: ملحقات فوتوشوب حديثة" />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )} 
                                        ...
                                        <FormField 
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel className="font-black">رابط صورة المعاينة</FormLabel>
                                                    <FormControl>
                                                        <Input {...field} className="h-12 rounded-xl" placeholder="https://..." />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )} 
                                        ...
                                        <div className="bg-muted/30 p-6 rounded-[2rem] space-y-5 border-2 border-dashed border-primary/10">
                                            ...
                                            {itemForm.watch('displayStyle') === 'style1' && (
                                                <FormField 
                                                    control={itemForm.control} 
                                                    name="downloadUrl" 
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel className="font-black text-xs">رابط التحميل المباشر</FormLabel>
                                                            <FormControl>
                                                                <Input {...field} className="h-11 rounded-xl bg-background" />
                                                            </FormControl>
                                                        </FormItem>
                                                    )} 
                                                />
                                            ...
                                            {itemForm.watch('displayStyle') === 'style3' && (
                                                <div className="space-y-4">
                                                    <FormField 
                                                        ...
                                                        render={({ field }) => (
                                                            <FormItem>
                                                                <FormLabel className="font-black text-xs">نص البرومبت</FormLabel>
                                                                <FormControl>
                                                                    <Textarea {...field} className="h-32 rounded-xl bg-background" dir="ltr" />
                                                                </FormControl>
                                                            </FormItem>
                                                        )} 
                                                    />
                                            ...
                                            {itemForm.watch('displayStyle') === 'style4' && (
                                                <FormField 
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel className="font-black text-xs">رابط الفيديو (YouTube)</FormLabel>
                                                            <FormControl>
                                                                <Input {...field} className="h-11 rounded-xl bg-background" placeholder="https://youtube.com/..." />
                                                            </FormControl>
                                                        </FormItem>
                                                    )} 
                                                />
                                            ...
                                            {itemForm.watch('displayStyle') === 'style6' && (
                                                <div className="space-y-4">
                                                    <FormField 
                                                        ...
                                                        render={({ field }) => (
                                                            <FormItem>
                                                                <FormLabel className="font-black text-xs">رابط الموقع</FormLabel>
                                                                <FormControl>
                                                                    <Input {...field} className="h-11 rounded-xl bg-background" placeholder="https://example.com" />
                                                                </FormControl>
                                                            </FormItem>
                                                        )} 
                                                    />
                                                    <FormField 
                                                        ...
                                                        render={({ field }) => (
                                                            <FormItem>
                                                                <FormLabel className="font-black text-xs">وصف قصير للموقع</FormLabel>
                                                                <FormControl>
                                                                    <Textarea {...field} className="h-20 rounded-xl bg-background" placeholder="اكتب وصفاً جذاباً هنا..." />
                                                                </FormControl>
                                                            </FormItem>
                                                        )} 
                                                    />
                                                ...
                                                <FormItem>
                                                    ...
                                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                        ...
                                                        <SelectContent className="rounded-xl">
                                                            ...
                                                            <SelectItem value="pro">برو (مشتركين فقط)</SelectItem>
                                                        </SelectContent>
                                                    </Select>
                                                </FormItem>
                                            )} 
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="flex flex-row items-center justify-between rounded-xl border p-4 shadow-sm bg-muted/10">
                                                    <div className="space-y-0.5">
                                                        ...
                                                        <FormDescription className="text-[10px]">تظهر شارة "جديد" للمستخدمين.</FormDescription>
                                                    </div>
                                                    <FormControl>
                                        ...
                                        <Button 
                                            type="submit" 
                                            className="w-full h-16 text-lg font-black rounded-3xl shadow-xl shadow-primary/20" 
                                            disabled={itemForm.formState.isSubmitting}
                                        >
                                            {itemForm.formState.isSubmitting ? <Loader2 className="animate-spin" /> : (isAdmin ? "نشر المحتوى فوراً" : "إرسال للمراجعة")}
                                        </Button>
                                    </form>
                                </Form>
                            ) : (
                    ...
                    <Card className="xl:col-span-7 shadow-xl rounded-[2.5rem] border-none bg-muted/10 overflow-hidden">
                        <CardHeader className="p-8">
                            <CardTitle className="text-xl font-black">المحتوى الحالي في القسم</CardTitle>
                        </CardHeader>
                            ...
                            <ScrollArea className="h-[750px] pr-4 -mr-4">
                                <div className="space-y-4">
                                    {isLoadingItems ? (
                                        ...
                                        <p className="text-center py-20 text-muted-foreground font-bold">لا يوجد محتوى في هذا القسم حالياً</p>
                                    ) : (
                                        currentItems?.map((item, idx) => (
                                            <div key={item.id} className="flex items-center gap-4 p-4 bg-card rounded-[1.5rem] border shadow-sm group hover:border-primary/30 transition-all">
                                                ...
                                                <div className="h-16 w-16 rounded-2xl bg-muted overflow-hidden border-2 border-muted flex-shrink-0 relative">
                                                    {item.imageUrl && <img src={item.imageUrl} className="object-cover h-full w-full" alt="" />}
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Badge variant={item.status === 'pending' ? 'destructive' : 'secondary'} className="text-[8px] font-bold h-4">
                                                            {item.status === 'pending' ? 'قيد المراجعة' : 'نشط ومفعل'}
                                                        </Badge>
                                                        <Badge variant="outline" className="text-[8px] font-bold h-4">الترتيب: {idx}</Badge>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-primary rounded-xl">
                                                        <Edit2 className="h-4 w-4" />
                                                    </Button>
                                                    {isAdmin && (
                                                        <Button 
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className="flex flex-col gap-0.5">
                                                    <Button variant="ghost" size="icon" className="h-6 w-6" disabled={idx === 0} onClick={() => moveItemInFirestore(pricingPlans, idx, 'up', ['pricingPlans'])}>
                                                        <ChevronUp className="h-4 w-4" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" className="h-6 w-6" disabled={idx === (pricingPlans?.length || 0) - 1} onClick={() => moveItemInFirestore(pricingPlans, idx, 'down', ['pricingPlans'])}>
                                                        <ChevronDown className="h-4 w-4" />
                                                    </Button>
                                                </div>
                                            </div>
            ...
            {isAdmin && (
                <>
                            ...
                            {isLoadingReview ? <Loader2 className="animate-spin mx-auto mt-10" /> : reviewItems.length === 0 ? <p className="col-span-full text-center py-20 bg-muted/20 rounded-[2.5rem] font-bold">كل شيء رائع! لا توجد منشورات بانتظار المراجعة.</p> : reviewItems.map(item => (
                                <Card key={item.id} className="overflow-hidden rounded-[2rem] border-2 shadow-xl">
                                    ...
                                    <CardHeader className="p-6">
                                        ...
                                        <CardDescription className="font-bold text-xs text-primary">
                                            القسم: {categories?.find(c => c.id === item.categoryId)?.name}
                                        </CardDescription>
                                    </CardHeader>
                                    <CardFooter className="p-4 pt-0 gap-2">
                                        <Button className="flex-1 bg-green-600 hover:bg-green-700 h-12 rounded-xl font-bold" onClick={() => updateDocumentNonBlocking(doc(firestore!, 'categories', item.categoryId, 'items', item.id), { status: 'approved' })}>
                                            <CheckCircle className="ml-2 h-4 w-4" /> قبول
                                        </Button>
                            ...
                            <Card className="xl:col-span-7 shadow-xl rounded-[2.5rem]">
                                <CardHeader className="p-8 pb-4">
                                    <CardTitle className="text-2xl font-black flex items-center gap-3">
                                        <BellRing className="h-7 w-7 text-primary" /> إرسال تنبيه جديد
                                    </CardTitle>
                                    <CardDescription>سيتم إشعار كافة مستخدمي التطبيق فوراً بهذا التنبيه.</CardDescription>
                                </CardHeader>
                                <CardContent className="p-8 pt-4">
                                    <Form {...notifForm}>
                                        <form onSubmit={notifForm.handleSubmit(async (values) => {
                                            if (!firestore) return;
                                            await addDocumentNonBlocking(collection(firestore, 'notifications'), { ...values, createdAt: serverTimestamp() });
                                            toast({ title: "تم إرسال الإشعار بنجاح" });
                                            notifForm.reset();
                                        })} className="space-y-6">
                                            <FormField control={notifForm.control} name="title" render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel className="font-black">عنوان الإشعار</FormLabel>
                                                    <FormControl><Input {...field} className="h-14 rounded-2xl" placeholder="مثال: تم إضافة قسم جديد للمصممين!" /></FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )} />
                                            <FormField control={notifForm.control} name="description" render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel className="font-black">محتوى الإشعار</FormLabel>
                                                    <FormControl><Textarea {...field} className="h-40 rounded-2xl" placeholder="اكتب تفاصيل الإشعار بوضوح هنا..." /></FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )} />
                                            <Button type="submit" className="w-full h-16 text-lg font-black rounded-3xl" disabled={notifForm.formState.isSubmitting}>
                                                {notifForm.formState.isSubmitting ? <Loader2 className="animate-spin" /> : <><Send className="ml-2 h-5 w-5" /> إرسال الإشعار الآن</>}
                                            </Button>
                                        </form>
                                    </Form>
                                </CardContent>
                                        ...
                                        <div className="space-y-4">
                                            {notifications?.map(notif => (
                                                <div key={notif.id} className="p-5 bg-card rounded-2xl border shadow-sm relative group">
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive absolute top-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => deleteDocumentNonBlocking(doc(firestore!, 'notifications', notif.id))}>
                                                        <Trash2 className="h-4 w-4" />
                                                    </Button>
                    ...
                    <TabsContent value="settings" className="space-y-8 m-0 animate-in fade-in duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <Card className="rounded-[2.5rem] shadow-lg border-none">
                                <CardHeader className="p-8">
                                    <CardTitle className="flex items-center gap-3 font-black">
                                        <Palette className="h-6 w-6 text-primary" /> مظهر المنصة
                                    </CardTitle>
                                    <CardDescription>تخصيص الألوان الأساسية للموقع (HSL)</CardDescription>
                                </CardHeader>
                                <CardContent className="p-8 pt-0 space-y-6">
                                    <div className="space-y-3">
                                        <label className="text-xs font-black px-1">اللون الأساسي (الوضع الفاتح):</label>
                                        <div className="flex gap-3">
                                            <Input value={themeConfig?.primaryColor || ''} onChange={(e) => updateConfig('theme', { primaryColor: e.target.value })} placeholder="350 72% 51%" className="h-12 rounded-xl" />
                                            <div className="w-12 h-12 rounded-xl border-2 shadow-inner" style={{ backgroundColor: `hsl(${themeConfig?.primaryColor})` }} />
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card className="rounded-[2.5rem] shadow-lg border-none">
                                <CardHeader className="p-8">
                                    <CardTitle className="flex items-center gap-3 font-black">
                                        <Gift className="h-6 w-6 text-primary" /> نظام المكافآت
                                    </CardTitle>
                                    <CardDescription>إعداد قواعد الترقية وتوليد الأكواد.</CardDescription>
                                </CardHeader>
                                <CardContent className="p-8 pt-0 grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase">نقاط ترقية برو</label>
                                        <Input type="number" value={refConfig?.pointsForProUpgrade || 500} onChange={(e) => updateConfig('referral', { pointsForProUpgrade: parseInt(e.target.value) })} className="h-12 rounded-xl" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase">نقاط كل إحالة</label>
                                        <Input type="number" value={refConfig?.pointsPerReferral || 50} onChange={(e) => updateConfig('referral', { pointsPerReferral: parseInt(e.target.value) })} className="h-12 rounded-xl" />
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                    ...
                    <TabsContent value="payments" className="space-y-8 m-0 animate-in fade-in duration-500">
                        <div className="flex justify-between items-center"><h2 className="text-3xl font-black">طرق الدفع</h2><Button className="h-14 px-8 rounded-2xl font-black shadow-lg" onClick={() => { paymentForm.reset(); setEditingPayment({ id: '' } as any); }}><Plus className="ml-2 h-5 w-5" /> إضافة طريقة دفع</Button></div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {paymentMethods?.map((method, idx) => (
                                <Card key={method.id} className="rounded-[2rem] border-2 border-primary/5 shadow-sm overflow-hidden">
                                    <CardHeader className="p-6 pb-4 bg-muted/20 flex-row items-center gap-4 space-y-0">
                                        <div className="flex flex-col gap-0.5">
                                            <Button variant="ghost" size="icon" className="h-6 w-6" disabled={idx === 0} onClick={() => moveItemInFirestore(paymentMethods, idx, 'up', ['paymentMethods'])}>
                                                <ChevronUp className="h-4 w-4" />
                                            </Button>
                                            <Button variant="ghost" size="icon" className="h-6 w-6" disabled={idx === (paymentMethods?.length || 0) - 1} onClick={() => moveItemInFirestore(paymentMethods, idx, 'down', ['paymentMethods'])}>
                                                <ChevronDown className="h-4 w-4" />
                                            </Button>
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-0"><CardTitle className="text-lg font-black truncate">{method.name}</CardTitle><div className="flex items-center gap-2 mt-1"><Badge variant="outline" className="text-[8px] font-black h-4 px-2">{method.country}</Badge></div></div>
          ...
          <DialogContent dir="rtl" className="max-w-md rounded-[2.5rem] border-none shadow-2xl p-8">
              ...
              <Form {...catForm}>
                  <form onSubmit={catForm.handleSubmit(onUpdateCategory)} className="space-y-6">
                      ...
                      <DialogFooter className="pt-4"><Button type="submit" className="w-full h-14 text-lg font-black rounded-2xl">حفظ القسم الآن</Button></DialogFooter>
                  </form>
              </Form>
          </DialogContent>
      </Dialog>
          ...
          <DialogContent dir="rtl" className="max-w-md rounded-[2.5rem] border-none shadow-2xl p-8">
              ...
              <Form {...paymentForm}>
                  <form onSubmit={paymentForm.handleSubmit(onUpdatePayment)} className="space-y-5">
                      ...
                      <DialogFooter className="pt-4"><Button type="submit" className="w-full h-14 font-black rounded-2xl">تأكيد الإعدادات</Button></DialogFooter>
                  </form>
              </Form>
          </DialogContent>
      </Dialog>
          ...
          <DialogContent dir="rtl" className="max-w-md rounded-[2.5rem] border-none shadow-2xl p-8">
              ...
              <Form {...userForm}>
                  <form onSubmit={userForm.handleSubmit(onAddUser)} className="space-y-6">
                      ...
                      <FormField control={userForm.control} name="role" render={({ field }) => (
                          <FormItem>
                              ...
                              <Select onValueChange={field.onChange} defaultValue={field.value}>
                                  <FormControl><SelectTrigger className="h-12 rounded-xl"><SelectValue /></SelectTrigger></FormControl>
                                  <SelectContent className="rounded-xl">
                                      <SelectItem value="pro">عضو برو</SelectItem>
                      ...
                      <DialogFooter className="pt-4"><Button type="submit" className="w-full h-14 text-lg font-black rounded-2xl" disabled={userForm.formState.isSubmitting}>
                          {userForm.formState.isSubmitting ? <Loader2 className="animate-spin" /> : "إضافة المستخدم الآن"}
                      </Button></DialogFooter>
                  </form>
              </Form>
          </DialogContent>
