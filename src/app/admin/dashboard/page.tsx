...
import type { Category, ContentItem, PricingPlan, ThemeConfig, Notification, ShareLinkConfig, RequestDesignConfig, WhitelistEntry, PaymentMethod } from '@/lib/definitions';
                    ...
                    <div className="flex gap-2">
                                ...
                                <Form {...catForm}>
                                    <form onSubmit={catForm.handleSubmit(onUpdateCategory)} className="space-y-5 pt-4">
                                        <FormField control={catForm.control} name="name" render={({ field }) => (
                                            <FormItem><FormLabel className="font-black text-xs">اسم القسم</FormLabel><FormControl><Input {...field} className="h-12 rounded-xl" /></FormControl></FormItem>
                                        )} />
                                        <FormField control={catForm.control} name="fileTypes" render={({ field }) => (
                                            <FormItem><FormLabel className="font-black text-xs flex items-center gap-2"><FileCode className="h-4 w-4 text-primary" /> صيغ الملفات (اختياري)</FormLabel><FormControl><Input {...field} placeholder="مثل: PSD, AI" className="h-12 rounded-xl" /></FormControl></FormItem>
                                        )} />
                                        <FormField control={catForm.control} name="displayStyle" render={({ field }) => (
                                            <FormItem><FormLabel className="font-black text-xs">نمط العرض</FormLabel>
                                        ...
                                        <div className="p-4 bg-muted/50 rounded-2xl flex items-center justify-between">
                                            <div className="space-y-0.5"><p className="text-xs font-black">وضع الصيانة</p><p className="text-[10px] text-muted-foreground">يخفي القسم عن المستخدمين</p></div>
                        ...
                        {selectedParentId && (
                            <Dialog open={isAddingItem} onOpenChange={setIsAddingItem}>
                                <DialogTrigger asChild>
                                    <Button size="lg" variant="outline" className="rounded-2xl font-black px-6 border-2"><Send className="ml-2 h-5 w-5" /> منشور جديد</Button>
                                </DialogTrigger>
                                <DialogContent className="max-w-md rounded-[2.5rem] p-8" dir="rtl">
                                    <DialogHeader><DialogTitle className="text-xl font-black">نشر محتوى</DialogTitle></DialogHeader>
                                    <Form {...itemForm}>
                                        <form onSubmit={itemForm.handleSubmit(onAddItem)} className="space-y-4 pt-4">
                                            <FormField control={itemForm.control} name="title" render={({ field }) => (<FormItem><FormLabel className="text-xs font-black">العنوان</FormLabel><FormControl><Input {...field} className="h-12 rounded-xl" /></FormControl></FormItem>)} />
                                            <FormField control={itemForm.control} name="imageUrl" render={({ field }) => (<FormItem><FormLabel className="text-xs font-black">رابط الصورة</FormLabel><FormControl><Input {...field} className="h-12 rounded-xl text-left" dir="ltr" /></FormControl></FormItem>)} />
                                            {currentCategory?.displayStyle === 'style3' && (
                                                <FormField control={itemForm.control} name="prompt" render={({ field }) => (<FormItem><FormLabel className="text-xs font-black">البرومبت</FormLabel><FormControl><Textarea {...field} className="rounded-xl font-mono text-xs" dir="ltr" /></FormControl></FormItem>)} />
                                            )}
                            ...
                            <Card key={cat.id} className="rounded-[2.5rem] border-none shadow-xl shadow-gray-200/50 overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
                                <CardHeader className="bg-primary p-8 text-primary-foreground relative overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start relative z-10">
                                        <div><CardTitle className="text-xl font-black">{cat.name}</CardTitle><Badge className="mt-2 bg-white/20 text-white border-none font-bold uppercase text-[9px]">{cat.displayStyle}</Badge></div>
                                ...
                                <CardFooter className="p-4 bg-muted/30 flex gap-2">
                                    ...
                                    {isAdmin && <Button variant="ghost" size="icon" className="h-11 w-11 rounded-xl text-destructive hover:bg-destructive/10" onClick={() => confirm("حذف القسم؟") && deleteDocumentNonBlocking(doc(firestore!, 'categories', cat.id))}><Trash2 className="h-4 w-4" /></Button>}
                                </CardFooter>
                            </Card>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="space-y-3">
                                {subCategories.length === 0 ? (<div className="text-center py-12 bg-white rounded-[2rem] border-2 border-dashed opacity-40"><p className="text-xs font-bold">لا توجد أقسام فرعية</p></div>) : (
                        ...
                        <div className="lg:col-span-8 space-y-6">
                            ...
                            <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white"><ScrollArea className="h-[600px]">{currentItems.length === 0 ? (<div className="text-center py-32 opacity-20"><Send className="h-16 w-16 mx-auto mb-4" /><p className="font-black">لم يتم إضافة محتوى بعد</p></div>) : (
                                <div className="divide-y">{currentItems.map((item, idx) => (
                                    <div key={item.id} className="flex items-center gap-4 p-5 hover:bg-secondary/20 transition-colors group">
                                        ...
                                        <div className="flex-1 min-w-0"><p className="font-black text-sm truncate">{item.title}</p><div className="flex items-center gap-2 mt-1">{item.isNew && <Badge className="bg-green-500 text-white text-[8px] h-4">جديد</Badge>}<span className="text-[10px] opacity-40 font-bold uppercase">{item.visibility}</span></div></div>
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">{isAdmin && <Button variant="ghost" size="icon" className="text-destructive h-10 w-10 hover:bg-destructive/10" onClick={() => confirm("حذف؟") && deleteDocumentNonBlocking(doc(firestore!, 'categories', selectedParentId!, 'items', item.id))}><Trash2 className="h-4 w-4" /></Button>}</div>
                                    </div>
                                ))}</div>)}</ScrollArea></Card>
                        </div>
                    </div>
        ...
        {activeTool === 'plans' && (
            <div className="space-y-8 animate-in fade-in duration-500">
                <div className="flex items-center justify-between">
                    <div><h2 className="text-2xl font-black">باقات الاشتراك</h2><p className="text-xs text-muted-foreground font-medium">إدارة خطط الأسعار.</p></div>
                    <Dialog open={!!editingPlan} onOpenChange={(o) => !o && setEditingPlan(null)}>
                        <DialogTrigger asChild><Button size="lg" className="rounded-2xl font-black px-6 shadow-xl shadow-primary/20" onClick={() => { planForm.reset({ name: '', price: '', currency: 'ر.س', features: '', isFeatured: false, enabled: true }); setEditingPlan({ id: '' } as any); }}><Plus className="ml-2 h-5 w-5" /> باقة جديدة</Button></DialogTrigger>
                        <DialogContent className="max-w-md rounded-[2.5rem] p-8" dir="rtl">
                            <DialogHeader><DialogTitle className="text-xl font-black">إعدادات الباقة</DialogTitle></DialogHeader>
                            <Form {...planForm}><form onSubmit={planForm.handleSubmit(onSavePlan)} className="space-y-4 pt-4">
                ...
                <Tabs defaultValue="appearance" className="w-full space-y-8" dir="rtl">
                    ...
                    <TabsContent value="appearance">
                        <Card className="rounded-[2.5rem] p-8 border-none shadow-xl">
                            ...
                            <Form {...themeForm}><form onSubmit={themeForm.handleSubmit(async (v) => { await setDoc(doc(firestore!, 'appConfig', 'theme'), v, {merge:true}); toast({title:"تم الحفظ"}); })} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <FormField control={themeForm.control} name="primaryColor" render={({ field }) => (<FormItem><FormLabel className="font-black">اللون الأساسي (الفاتح)</FormLabel><FormControl><div className="flex gap-2"><Input {...field} className="font-mono text-left" dir="ltr" /><div className="h-10 w-10 rounded-xl border" style={{ backgroundColor: `hsl(${field.value})` }} /></div></FormControl></FormItem>)} />
                                    <FormField control={themeForm.control} name="primaryColorDark" render={({ field }) => (<FormItem><FormLabel className="font-black">اللون الأساسي (الليلي)</FormLabel><FormControl><div className="flex gap-2"><Input {...field} className="font-mono text-left" dir="ltr" /><div className="h-10 w-10 rounded-xl border bg-black" style={{ backgroundColor: `hsl(${field.value})` }} /></div></FormControl></FormItem>)} />
                                </div>
                    ...
                    <TabsContent value="notifications">
                        <Card className="rounded-[2.5rem] p-8 border-none shadow-xl">
                            ...
                            <Form {...notifyForm}><form onSubmit={notifyForm.handleSubmit(async (v) => { await addDocumentNonBlocking(collection(firestore!, 'notifications'), {...v, createdAt:serverTimestamp()}); toast({title:"تم الإرسال"}); notifyForm.reset(); })} className="space-y-4">
                                <FormField control={notifyForm.control} name="title" render={({ field }) => (<FormItem><FormLabel className="font-black">العنوان</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                <FormField control={notifyForm.control} name="description" render={({ field }) => (<FormItem><FormLabel className="font-black">النص</FormLabel><FormControl><Textarea {...field} /></FormControl></FormItem>)} />
                                <Button type="submit" className="h-14 px-8 rounded-2xl font-black">إرسال الآن</Button>
                            </form></Form>
                        </Card>
                    </TabsContent>
                    ...
                    <TabsContent value="payments" className="space-y-6">
                        <div className="flex items-center justify-between">
                            <h3 className="font-black text-lg">طرق الدفع</h3>
                            <Dialog open={!!editingPayment} onOpenChange={(o) => !o && setEditingPayment(null)}>
                                <DialogTrigger asChild><Button className="rounded-xl font-black" onClick={() => { paymentForm.reset({ name: '', icon: 'Wallet', link: '', isUrl: true, country: 'ALL', enabled: true }); setEditingPayment({ id: '' } as any); }}><Plus className="ml-2 h-4 w-4" /> طريقة دفع</Button></DialogTrigger>
                                <DialogContent className="max-w-md rounded-[2.5rem] p-8" dir="rtl">
                                    <DialogHeader><DialogTitle className="font-black">إعدادات وسيلة الدفع</DialogTitle></DialogHeader>
                                    <Form {...paymentForm}><form onSubmit={paymentForm.handleSubmit(onSavePayment)} className="space-y-4 pt-4">
                                        ...
                                        <Button type="submit" className="w-full h-14 rounded-2xl font-black">حفظ وسيلة الدفع</Button>
                                    </form></Form>
                                </DialogContent>
                            </Dialog>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {payments?.map((method) => (
                                <Card key={method.id} className="rounded-2xl border-none shadow-md">
                                    <CardHeader className="flex flex-row items-center justify-between p-4 pb-2">
                                        <div className="flex items-center gap-3"><Wallet className="h-5 w-5 text-primary" /><CardTitle className="text-sm font-black">{method.name}</CardTitle></div>
                                        <div className="flex items-center gap-1">
                                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingPayment(method); paymentForm.reset(method); }}><Edit2 className="h-3.5 w-3.5" /></Button>
                                            {isAdmin && <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => confirm("حذف؟") && deleteDocumentNonBlocking(doc(firestore!, 'paymentMethods', method.id))}><Trash2 className="h-3.5 w-3.5" /></Button>}
                                        </div>
                                    </CardHeader>
                                    <CardContent className="p-4 pt-0"><p className="text-[10px] text-muted-foreground truncate">{method.link}</p><Badge variant="outline" className="mt-2 text-[8px]">{method.country}</Badge></CardContent>
                                </Card>
                            ))}
                        </div>
                    </TabsContent>
